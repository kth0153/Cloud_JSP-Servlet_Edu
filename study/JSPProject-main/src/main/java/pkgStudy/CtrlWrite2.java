...
//-------------------------------------------------------------------------------------------------------------------------
public class CtrlWrite2 extends HttpServlet {
...
//		   [^0-9] : 문자열 중에서 0 ~9 를 제외한 문자열 (a-z, A-Z)가 해당
		 ...
		 * if(part.getSubmittedFileName()!=null) { // 다른 입력 요소 제외 파일명이 전달된 것만 처리(파일이
		 * 있을경우) System.out.println(part.getSubmittedFileName());
		 * 
		 * String uploadFilePath =
		 * "C:\\Users\\KoTaehoon\\Desktop\\cloud_edu\\JSP_Servlet\\study\\JSPProject-main\\src\\main\\webapp\\fileupload\\";
		 * // 맨뒤 \\ 생략하거나 넣거나 가능, 업로드 파일 경로 String regFileName="";
		 * 
		 * orgName = part.getSubmittedFileName(); // 요청된 파일의 이름 가져오는 것
		 * if(!orgName.isEmpty()) { // 파일 이름이 있을 경우에만 firstName =
		 * orgName.substring(0,orgName.lastIndexOf(".")); // '.'을 기준으로 0~'.' 까지 파일 앞 이름
		 * => 01_JSP&Servlet1 exName = orgName.substring(orgName.indexOf(".")); // '.' ~
		 * 마지막 인덱스 까지 파일 뒤쪽 이름 => .pdf } // ===================파일 중복 체크 후 새 파일명
		 * 생성=================== File file = new File(uploadFilePath + orgName); // 업로드
		 * 경로 + 맨처음 받아온 파일명 int cnt=1; // 중복된 파일명 뒤에 붙일 값 regFileName = orgName; // 중복되지
		 * 않는 파일이 업로드 된 경우 대비 while(file.exists()) { // file.exists() : 파일의 존재 유무 확인 /
		 * 업로드 된 파일 명과 동일한 것이 있는지 판단 // 중복된 파일이 있는 경우 종복된 파일명이 존재하지 않을 때 까지 파일명 변경
		 * 
		 * // 1. 파일명 만들기 regFileName = firstName + (cnt++) + exName; // 중복되는 파일이 있는 경우
		 * 숫자를 붙여줘서 파일 중복을 없앤다 file = new File(uploadFilePath + regFileName); // 기존의 파일
		 * + 새로 만들어질 파일명
		 * 
		 * } // e:while(file.exists()) // uploadFilePath에 지정된 폴더로 클라이언트에서 전송된 파일을 // 새로
		 * 만들어진 이름으로 저장 file = new File(uploadFilePath); if(!file.exists()) { // 해당 경로에
		 * 폴더가 없다면 file.mkdirs(); // 해당 경로에 폴더를 생성 } fileSize = part.getSize(); // Part
		 * 에서 전송된 파일의 크기를 반환 받음 // out.println(regFileName); part.write(uploadFilePath +
		 * regFileName); // 새로 만들어진 이름으로 파일 저장
		 * 
		 * // ===================파일 중복 체크 후 새 파일명 생성 끝===================
		 * 
		 * // ========= DB Connection 호출 일반적 사용 ================= idx =
		 * bbs.getGroupIdx(); System.out.println("idx 값 : " +idx); if(idx>0) { // P/K 값을
		 * 정상적으로 받아온 경우, -1은 장애발생을 의미 // idx, 1, orgName, regFileName, fileSize String
		 * strSQL =
		 * "insert into TBOARD_FILE(FIDX,FBOARDIDX,FFILENAME,FSAVENAME,FFILESIZE)" +
		 * "values('"+idx+"','1','"+orgName+"','"+regFileName+"','"+fileSize+"');";
		 * //out.println(strSQL); //bbs.addFile(strSQL);
		 * out.println(bbs.addFile(strSQL)); } // ========= DB Connection 호출 일반적 사용 끝
		 * =================
		 * 
		 * if(idx>0) { // P/K 값을 정상적으로 받아온 경우, -1은 장애발생을 의미 strSQL += "('"+(idx++)
		 * +"','1','"+orgName+"','"+regFileName+"','"+fileSize+"'),"; // idx 값을 증가 시켜줘야
		 * 함!!! }// e:if(idx>0)
		 * 
